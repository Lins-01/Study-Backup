...
def seq_convert_example(
        example, label_list, tokenizer=None, max_seq_length=512, dynamic_max_length: Optional[List[int]] = None,
        **kwargs
):
...
def read(data_path):
    # 打开文件指定utf8，下面json.load也就不会乱码了。已经读出存在变量line中
    with open(data_path, 'r', encoding='utf-8') as f:
        for line in f:
            # line是字符串
            # {str} '{"id": 0, "sentence1": "<返回微博正文", "sentence2": " 1、SIGN UPN"}'
            # json.load将字符串转为字典
            line = json.loads(line)
            id, sentence1, sentence2 = line['id'], line['sentence1'], line['sentence2']
            yield {'id': id, 'sentence1': sentence1, 'sentence2': sentence2}
