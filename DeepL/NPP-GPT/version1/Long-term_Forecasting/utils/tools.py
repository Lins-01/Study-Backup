...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, path):
        score = -val_loss
        # 之前没存过，第一次，当前为best score，然后存下模型
        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, path)
        # 当前分数线小于best score+ 阈值，计数+1，并打印，如果三次，跳出，
...
def test(model, test_data, test_loader, args, device, itr):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(test_loader)):
            ...
            for j in range(pred.shape[0]):
                # 计算每个样本的RMSE，选择出最小的样本
                rmse = RMSE(pred[j], true[j])
                if min_rmse > rmse:
                    min_rmse = rmse
                    min_pred = pred[j]
                    min_true = true[j]
            ...
            #
            #
            #     # 第70个batch的第70个样本的第一个特征
            #     pred70 = pred[70, :, 0]
            #     true70 = true[70, :, 0]
            #     plt.figure(figsize=(12,6))
            #     plt.plot(pred70, label='pred')
            #     plt.plot(true70, label='vol_170101010.tempf_true')
            #     plt.legend() # 显示图例,即label
            #     plt.savefig('my_e1_itr1_seq1024_label256_predict384.png')
            #     plt.show()
            #     print("here")
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(min_pred, label='pred')
    plt.plot(min_true, label='vol_170101010.tempf_true')
    plt.legend()  # 显示图例,即label
    plt.savefig('min_true_e10.png')
    plt.show()
    print("here")
