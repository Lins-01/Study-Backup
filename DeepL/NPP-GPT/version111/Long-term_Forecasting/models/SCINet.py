...
class Interactor(nn.Module):
    def __init__(self, in_planes=9, splitting=True,
...
class SCINet(nn.Module):
    def __init__(self, configs):
        ...
        self.kernel_size = 3
        self.dropout = 0.5
        self.single_step_output_One = 0 # 一次输出N步，还是只输出第N步
        self.concat_len = 0
        self.pe = True # ?
        self.RIN = False # ?
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dim', type=int, default=375)
    ...
    model = SCINet(output_len=args.horizon, input_len=args.window_size, input_dim=args.input_dim,
                   hid_size=args.hidden_size, num_stacks=1, num_levels=2, concat_len=0, groups=args.groups,
                   kernel=args.kernel, dropout=args.dropout, single_step_output_One=args.single_step_output_One,
                   positionalE=args.positionalEcoding, modified=True).cuda()

    x = torch.randn(64, 128, 375).cuda()
    y = model(x)
    print(y.shape)
