...
# 所有处理数据集的子类，都应该继承Dataset，并且重写__getitem__，使其可以用key访问value
...
def collate_context_bert_train(batch):
    ...
    for j in range(0, len(samples)):
        ...
        if len(sample['imgs'].shape) > 2:
            padding_size = (max_images_len - sample['imgs'].shape[0], sample['imgs'].shape[1], sample['imgs'].shape[2],
                            sample['imgs'].shape[3])
        else:
...
if __name__ == '__main__':
    ...
    val_dataset = NewsContextDatasetEmbs(data_items_val,
                                         r'E:\Document\CodeSpace\Data_set\Paddle2023IKCEST\queries_dataset_merge',
                                         'val')
