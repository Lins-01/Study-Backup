...
if __name__ == '__main__':
    model = LevelSCINet(in_planes=9, kernel_size=5, dropout=0.5, groups = 1, hidden_size = 1, INN = True).cuda()

    x = torch.randn(64, 128, 9).cuda()
    y = model(x)
    print(y[0].shape)
    print(y[1].shape)
...
# if __name__ == '__main__':
#     model = SCINet(input_len=128, output_len=12, input_dim=375,
#                    hid_size=1, num_stacks=1, num_levels=2, concat_len=0, groups=1,
#                    kernel=3, dropout=0.5, single_step_output_One=0,
#                    positionalE=True, modified=True).cuda()
#
